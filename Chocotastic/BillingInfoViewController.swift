...
private extension BillingInfoViewController {
  ...
  func setupCardImageDisplay() {
    cardType
      ...
      .disposed(by: disposeBag) // Ensure the observer’s disposal in thedisposeBag
  }
  
  func setupTextChangeHandling() {
    let creditCardValid = creditCardNumberTextField
      .rx
      .text // text is another RxCocoa extension, this time to UITextField
      .observeOn(MainScheduler.asyncInstance)
      .distinctUntilChanged()
      // Throttle the input to set up the validation to run based on the interval defined
 ...
 // Transform the throttled input by applying it to validate(cardText:) provided by the class. If the card input is valid, the ultimate value of the observed boolean will be true.
    ...
    let everythingValid = Observable
      .combineLatest(creditCardValid, expirationValid, cvvValid) {
        $0 && $1 && $2 //All must be true. generated Observable, called everythingValid, is either true or false, depending on whether all three inputs are valid.
    }
